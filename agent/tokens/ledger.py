...
import time
# Note: Assuming 'LedgerEntry' is correctly imported from your schema/local file
...
class TransactionLedger:
    ...
    def _init_database(self):
        """Initialize SQLite database (Using context manager)"""
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        
        # FIX: Use context manager for safe connection/commit/close
    ...
    def add_entry(self, entry: LedgerEntry):
        """Add entry to ledger (Using context manager)"""
        # FIX: Use context manager to prevent database locking
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def get_entries(self, 
        ...
        """Query ledger entries (Using context manager)"""
        # FIX: Use context manager to prevent database locking
        with sqlite3.connect(self.db_path) as conn:
    ...
    def get_statistics(self) -> dict:
        """Get ledger statistics (Using context manager)"""
        # FIX: Use context manager to prevent database locking
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
