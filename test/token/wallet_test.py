...
import time
from dataclasses import dataclass, asdict # Assuming dataclass is now imported here

# Assuming schema is imported from schema.py or defined here
@dataclass
class LedgerEntry:
    """Single ledger entry"""
    entry_id: str
    ...
    def to_dict(self) -> dict:
        return asdict(self)
# End of assumed schema definition
...
class TransactionLedger:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # conn.commit() is automatically called by the context manager

    def add_entry(self, entry: LedgerEntry):
        """Add entry to ledger"""
        # Use context manager to prevent database locking issues
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def get_statistics(self) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Total spent (assumes withdrawals/slashes are recorded as negative amounts)
            cursor.execute('''
